...
lower_val = 0.0
upper_val = 0.0
dmm4095 = 0              # 0 = 4094, 1 = 4095, 2 = 4096
multiplikator = 1
ts = 1
rad = 0
limit_switch= 0
...
TEMP_RDT_TYPE_4094 = ["KITS90", "PT100"]

CAP_4094 = ["50nF", "500nF", "5µF", "50µF", "500µF", "5mF", "50mF"]
CAP_409x = ["2nF", "20nF", "200nF", "2µF", "20µF", "200µF", "10mF"]

VDC_4094 = ["50mV", "500mV", "5V", "50V", "500V", "1000V"]
VAC_4094 = ["500mV", "5V", "50V", "500V", "750V"]
ADC_4094 = ["500µA", "5mA", "50mA", "500mA", "5A", "10A"]
AAC_4094 = ["500µA", "5mA", "50mA", "500mA", "5A", "10A"]
...
RES_4094 = ["500Ω", "5KΩ", "50KΩ", "500KΩ", "5MΩ", "50MΩ"]
RES_4095 = ["600Ω", "6KΩ", "60KΩ", "600KΩ", "6MΩ", "60MΩ", "100MΩ"]
RES_4096 = ["200Ω", "2KΩ", "20KΩ", "200KΩ", "2MΩ", "10MΩ", "100MΩ"]

print('for a NETWORK or USB connection use \"python3 Multimeter-TCP.py\"')
...
found = 0
if ASRL == '0':
    oscilloscope = serial.Serial(RS232_PORT, baudrate=RS232_SPEED, bytesize=8, parity='N', stopbits=1, timeout = 0.5, xonxoff=0, rtscts=0)
    oscilloscope.write(str.encode('*IDN?\n'))
    leer = oscilloscope.readline()
    leer = leer.decode('utf-8')
    leer = leer.replace('"', '')
    leer = leer.strip('\n')
    leer = leer.rstrip()
    if "P4094" in leer or "2041" in leer or "7055" in leer:
        dmm4095 = 0
        found = 1
    if "P4095" in leer or "3041" in leer or "7060" in leer:
        dmm4095 = 1
        found = 1
    if "P4096" in leer or "3051" in leer or "7200" in leer:
        dmm4095 = 2
        found = 1
...
if ASRL == '1':
    ...
    for instrument in instruments:
        test_port = instrument.replace('::INSTR', '')
        test_port = test_port.replace('ASRL', '')
        print("Checking: ", test_port)
        if 'ASRL' in instrument:
            oscilloscope = serial.Serial(test_port, baudrate=RS232_SPEED, bytesize=8, parity='N', stopbits=1, timeout = 0.5, xonxoff=0, rtscts=0)
            try:
                oscilloscope.write(str.encode('*IDN?\n'))
                leer = oscilloscope.readline()
                leer = leer.decode('utf-8')
                ...
                leer = leer.strip('\n')
                leer = leer.rstrip()
                print("Resource: '" + test_port + "' is \"" + leer + '\"\n')
                if "P4094" in leer or "2041" in leer or "7055" in leer:
                    dmm4095 = 0
                    found = 1
                if "P4095" in leer or "3041" in leer or "7060" in leer:
                ...
                if "P4096" in leer or "3051" in leer or "7200" in leer:
                    ...
                    found = 1
                if leer !='':
                    RS232_PORT = test_port
                    break
            except serial.SerialException as e:
                oscilloscope = serial.Serial(test_port, baudrate=RS232_SPEED, bytesize=8, parity='N', stopbits=1, timeout = 1, xonxoff=0, rtscts=0)
                print('No connection to DMM: ' + test_port)
    del resources

# dmm4095 = 0           # TEST forced P4094
# dmm4095 = 2           # TEST forced P4096
...
def ende():
    ...
    oscilloscope.write(b'SYSTem:LOCal\n')
    print ("Window Ende")
    if save_flag == 1:
        save_flag = 0
        workbook.close()
...
class Ui(QtWidgets.QMainWindow):
    def __init__(self, *args, **kwargs):
        ...
        if usb_switch == 0:
            self.setWindowTitle(leer + " RS232::BAUD:" + RS232_SPEED + " on Port: " + RS232_PORT)
        elif usb_switch == 1:
        ...
        if dmm4095 == 0:
            print ("DMM 4½ Digits 55000 Counts")
            if 'blue' in SCREEN:
                self.setFixedSize(745, 258)
                self.Logo.setProperty("text","PeakTech P4094")
        elif dmm4095 == 1:
...
#        einheit = self.tcpip('FUNC1?')
    ...
    def reset_on_error(self):
        ...
        oscilloscope.close()
        oscilloscope = serial.Serial(RS232_PORT, baudrate=RS232_SPEED, bytesize=8, parity='N', stopbits=1, timeout = 1, xonxoff=0, rtscts=0)
        self.tcpip('SHOW ON')
        self.tcpip('CALCulate:NULL:OFFSet 0.00')
        self.tcpip('CALCulate:STATe OFF')
...
#            print (func_2+' <===> '+func_1)
    ...
    def tcpip(self, text_out):
        ...
        if usb_switch == 0:
            data = "sende..."
            if "?" in text_out:
                oscilloscope.write(str.encode(text_out))
                data = oscilloscope.readline()
        ...
        if "TEMP" in einheit:
            ...
            if temptype != '':
                if dmm4095 == 0:
                    if temptype == "NITS90":
                        temptype = "PT100"
                    temp_type_val = TEMP_RDT_TYPE_4094.index(temptype) + 1
...
#        self.db_widget.setVisible(False)
        ...
        if "TEMP" in einheit:
            ...
            if dmm4095 == 0:
                ...
                self.dial.setProperty("toolTip", "Sensor Selection " + str(TEMP_RDT_TYPE_4094))
                self.dial.setNotchesVisible(True)
                temptype = self.tcpip('TEMP:RTD:TYP?')
                if temptype == "NITS90":
                    temptype == "PT100"
            ...
            if dmm4095 == 0:
                self.dial.setProperty("toolTip", "Range Auto " + str(VAC_4094))
            elif dmm4095 == 1:
            ...
            if bereich != "1":
                if dmm4095 == 0:
                    self.small_1.setProperty("text", VAC_4094[int(otto)-1])
                elif dmm4095 == 1:
            ...
            if int(otto) == 1:
                if dmm4095 == 0:
                    otto1 = otto1 + " " + VAC_4094[int(otto)-1]
                elif dmm4095 == 1:
            ...
            if dmm4095 == 0:
                ...
                self.dial.setProperty("toolTip", "Range Auto " + str(VDC_4094))
            elif dmm4095 == 1:
            ...
            if bereich != "1":
                if dmm4095 == 0:
                    self.small_1.setProperty("text", VDC_4094[int(otto)-1])
                elif dmm4095 == 1:
            ...
            if int(otto) == 1:
                if dmm4095 == 0:
                    otto1 = otto1 + " " + VDC_4094[int(otto)-1]
                elif dmm4095 == 1:
            ...
            if dmm4095 == 0:
                self.dial.setRange(0,6)
                self.dial.setProperty("toolTip", "Range Auto " + str(AAC_4094))
                self.dial.setNotchesVisible(True)
                otto1 = "MANUAL"
                otto = self.tcpip('RANGE1?')
                if bereich != "1":
                    self.small_1.setProperty("text", AAC_4094[int(otto)-1])
                if bereich == "1":
                    otto1 = "AUTO"
                    self.small_1.setProperty("text", "AUTO")
                    self.dial.setValue(0)
                if int(otto) == 1:
                    otto1 = otto1 + " " + AAC_4094[int(otto)-1]
                    otto = "µA"
            ...
            self.dial.setRange(0,6)
            if dmm4095 == 0:
                self.dial.setProperty("toolTip", "Range Auto " + str(ADC_4094))
            elif dmm4095 == 1:
            ...
            if bereich != "1":
                if dmm4095 == 0:
                    self.small_1.setProperty("text", ADC_4094[int(otto)-1])
                elif dmm4095 == 1:
            ...
            if int(otto) == 1:
                if dmm4095 == 0:
                    otto1 = otto1 + " " + ADC_4094[int(otto)-1]
                elif dmm4095 == 1:
            ...
            if dmm4095 == 0:
                self.dial.setProperty("toolTip", "Range Auto " + str(RES_4094))
            ...
            if bereich != "1":
                if dmm4095 == 0:
                    self.small_1.setProperty("text", RES_4094[int(otto)-1])
                elif dmm4095 == 1:
            ...
            if int(otto) == 1:
                if dmm4095 == 0:
                    otto1 = otto1 + " " + RES_4094[int(otto)-1]
                elif dmm4095 == 1:
...
#                self.dial.setValue(4)
            ...
            self.dial.setRange(0,7)
            if dmm4095 == 0:
                self.dial.setProperty("toolTip", "Range Auto " + str(CAP_4094))
            elif dmm4095 == 1:
            ...
            if bereich != "1":
                if dmm4095 == 0:
                    self.small_1.setProperty("text", CAP_4094[int(otto)-1])
                elif dmm4095 == 1:
            ...
            if int(otto) == 1:
                if dmm4095 == 0:
                    otto1 = otto1 + " " + CAP_4094[int(otto)-1]
                elif dmm4095 >= 1:
    ...
    def dual_d(self):
      ...
      try:
        ...
        if "VOLT AC" in einheit2:
            ...
            if int(otto2) == 1:
                if dmm4095 == 0:
                    otto21 = otto21 + " " + VAC_4094[int(otto2)-1]
                elif dmm4095 == 1:
            ...
            if int(otto2) == 1:
                if dmm4095 == 0:
                    otto21 = otto21 + " " + VDC_4094[int(otto2)-1]
                elif dmm4095 == 1:
            ...
            if dmm4095 == 0:
                if int(otto2) == 1:
                    otto21 = otto21 + " " + ADC_4094[int(otto2)-1]
                    otto2 = "µA"
                    multiplikator = 1000000
                    nachkomma = 6
                if int(otto2) == 2:
